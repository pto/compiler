...
PRIVATE int (*Openp)(char *, int) = (int (*)(char *, int))open;
PRIVATE int (*Closep)(int) = close;
PRIVATE int (*Readp)(int, void *, unsigned) = read;

void ii_io(int (*open_funct)(char *name, int mode),
           int (*close_funct)(int handle),
           int (*read_funct)(int handle, void *target_buffer, unsigned len)) {
    /*
     * This function lets you modify the open(), close(), and read()
     * functions used by the i/o system. Your own routines must work like the
     * real open, close, and read (at least in terms of the external
     * interface. Open should return a number that can't be confused with
     * standard input (not 0).
     */
...
int ii_newfile(char *name) {
     ...
     * closed after successfully  opening the new one (but stdin isn't
     * closed).
     * 
     ...
     * free file descriptor must be available when newfile() is called. Note
     * in the open call that O_BINARY, which is needed in MS-DOS
...
uchar *
     ...
     * move sMark. I'm not doing this automatically because I might want to
...
{
    /*
     * Flush the input buffer. Do nothing if the current input character
     * isn't in the danger zone, otherwise move all unread characters to the
     * left end of the buffer and fill the remainder of the buffer. Note that
     * input() flushes the buffer willy-nilly if you read past the end of
     * buffer. Similarly, input_line() flushes the buffer at the beginning of
     * each line.
     * 
     * pMark	DANGER |   	   | Start_buf		       sMark eMark
     * |Next  End_buf |			||     |   ||	  | V V     V   VV
     * V +-----------------------+----------------+-------+ | this is already
     * read  | to be done yet | waste |
     * +-----------------------+----------------+-------+ | |       | |<-----
     * shift_amt ----->|<-- copy_amt -->| | |
     * | |<------------------ BUFSIZE ------------------->|
     * 
    ...
    if (Next >= DANGER || force) {
        ...
        if (shift_amt < MAXLEX) {       /* if(not enough room) */
            ...
            left_edge = ii_mark_start();        /* Reset start to current
                                                 * character */
            ii_mark_prev();
...
unsigned char *starting_at;
{
     ...
     * input file is not closed when EOF is reached. Buffers are read in
     * units of MAXLEX characters; it's an error if that many characters
     * cannot be read (0 is returned in this case). For example, if MAXLEX is
     * 1024, then 1024 characters will be read at a time. The number of
     ...
     * possible for read() to always return fewer than the number of
     * requested characters in MS-DOS untranslated-input mode, however (if
     * the file is opened without the O_BINARY flag). That's not a problem
     * here because the file is opened in binary mode, but it could cause
     * problems if you change from binary to text mode at some point.
     */
...
{
    /*
     * Return the nth character of lookahead, EOF if you try to look past end
     * of file, or 0 if you try to look past either end of the buffer. We
     * have to jump through hoops here to satisify the ANSI restriction that
     * a pointer can not go to the left of an array or more than one cell
     * past the right of an array. If we don't satisfy this restriction, then
     * the code won't work in the 8086 large or compact models. In the small
     * model---or in any machine without a segmented address space, you could
     * do a simple comparison to test for overflow: uchar *p = Next + n; if(
     * !(Start_buf <= p && p < End_buf ) overflow
     */
